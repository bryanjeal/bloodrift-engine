...
// Ground effect fragment shader - auras (swirling noise) and pyres (procedural fire).
//
// effect_type 0 = aura:  swirling simplex noise at 0.2 opacity, affinity-colored
// effect_type 1 = pyre:  fbm-based fire, warm gradient, 0.6 opacity

layout(location = 0) in vec2 frag_uv;
layout(location = 1) in vec4 frag_color;
layout(location = 2) in flat float frag_radius;
layout(location = 3) in flat float frag_time;
layout(location = 4) in flat float frag_effect_type;
layout(location = 5) in flat float frag_intensity;
...
// Fractional Brownian motion - two octaves for cheap swirling.
float fbm2(vec2 p) {
    float v = 0.0;
    float a = 0.5;
...
void main() {
    ...
    if (frag_effect_type < 0.5) {
        // === Aura: swirling noise ===
        float t = frag_time;

        // Rotate UV over time for swirl.
        float angle = t * 0.5;
        float ca = cos(angle);
        float sa = sin(angle);
        vec2 ruv = vec2(frag_uv.x * ca - frag_uv.y * sa,
                        frag_uv.x * sa + frag_uv.y * ca);

        // Sample noise at two scales for visual richness.
        float n = fbm2(ruv * 3.0 + t * 0.3);

        // Remap from [-1,1] to [0,1] and sharpen.
        n = smoothstep(-0.3, 0.7, n * 0.5 + 0.5);

        // Edge fade: smooth falloff from center to edge.
        float edge = 1.0 - smoothstep(0.4, 1.0, dist);

        float alpha = 0.2 * n * edge * frag_intensity;
        out_color = vec4(frag_color.rgb, alpha);

    } else {
        // === Pyre: procedural fire ===
        float t = frag_time;
